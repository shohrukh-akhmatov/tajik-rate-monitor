...
function renderReferenceRates(){
  ...
  for(const currency of ['USD','EUR','RUB','CNY','KZT']){
    ...
    if(q) rows.push(`<div class="history-item"><strong>NBT · ${currency}</strong><div class="change">${fmt(q.rate,6)} TJS · nominal ${q.nominal ?? '—'} · ${q.date || nbt.updated_at || 'date unavailable'}</div></div>`);
    else rows.push(`<div class="history-item"><strong>NBT · ${currency}</strong><div class="change">— not returned by API</div></div>`);
  }
  rows.push(`<div class="history-item"><strong>Alif API</strong><div class="change">Status: ${statusText(alif.status)} · ${Object.keys(alif.rates||{}).length ? Object.entries(alif.rates).map(([c,q])=>`${c}: ${fmt(q.buy,6)} / ${fmt(q.sell,6)}`).join(' · ') : (alif.note || alif.error || 'No recognized quotes')}</div></div>`);
  const banks = rr.banks || {};
  for(const [id,b] of Object.entries(banks)){
    const categoryRows = Object.entries(b.rates||{}).filter(([k])=>!k.startsWith('_')).flatMap(([category,quotes])=>Object.entries(quotes||{}).map(([currency,q])=>`${currency} ${fmt(q.buy,6)} / ${fmt(q.sell,6)} (${category})`));
    if(categoryRows.length) rows.push(`<div class="history-item"><strong>${b.name || id}</strong><div class="change">${categoryRows.join(' · ')}</div></div>`);
  }
...
async function rescanRates(){const button=$('#refresh');const previousGeneratedAt=data?.generated_at||null;button.disabled=true;button.setAttribute('aria-busy','true');setRefreshLabel('Starting scan…');$('#alert').classList.add('hidden');try{const response=await fetch(RESCAN_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}',cache:'no-store'});let payload={};try{payload=await response.json();}catch{}if(response.status===429)setRefreshLabel('Scan already running…');else if(!response.ok)throw new Error(payload?.message||payload?.error||`HTTP ${response.status}`);else setRefreshLabel('Scanning banks…');const updated=await waitForNewSnapshot(previousGeneratedAt);if(updated)setRefreshLabel('Scan complete ✓',2200);else{setRefreshLabel('Scan still pending',2600);$('#alert').textContent='The scan was triggered, but a newer deployed snapshot did not appear within 4 minutes. Check GitHub Actions for the workflow status.';$('#alert').classList.remove('hidden');}}catch(e){setRefreshLabel('Rescan failed',2400);$('#alert').textContent=`Could not start a rate rescan: ${e.message}`;$('#alert').classList.remove('hidden');}finally{button.disabled=false;button.removeAttribute('aria-busy');}}
