...
def fetch_json(url):
    for attempt in range(3):
        try:
            r=requests.get(url,params={'t':int(datetime.now().timestamp())},timeout=12,headers={'Cache-Control':'no-cache','User-Agent':'TajikRateMonitor/2.0'})
...
def choose_base(bank_code, bank, alif_base, alif_source, fallback_banks, last_valid_lookup):
    ...
    if base is not None:
        return base, bank_code, 'last_valid_route'
    return None, bank_code, 'missing'


def card_rub_row(bank_code, bank, base, src_bank, src_kind, rules, generated):
    """Build one 'Rates in Tajikistan' RUB card row. A last-valid base is published
    as stale so a downed bank's retained value is not marked fresh."""
    return {
        ...
        'status': 'stale' if src_kind == 'last_valid_route' else 'ok',
        'anomaly_code': None,
...
def main():
    ...
    for service_slug,service in rules['services'].items():
        for bank_code,coef in service.get('coefficients',{}).items():
            ...
            base,src_bank,src_kind=choose_base(bank_code,bank,alif_base,alif_source,fallback_banks,lambda b: last_valid_by_bank.get((service_slug,b)))
            if base is None:
                anomalies.append({'service_slug':service_slug,'bank_code':bank_code,'code':'MISSING_BASE','message':'No usable base after own quote and configured fallbacks.'}); continue
            old_base=None
            if src_bank=='alif':
