...
CURRENCIES = {
    "USD": (5.0, 20.0), "EUR": (7.0, 20.0), "RUB": (0.05, 0.20),
    "CNY": (0.5, 3.0), "KZT": (0.01, 0.20),
}
...
def nbt_rates() -> dict[str, Any]:
    out: dict[str, Any] = {"source": NBT_API, "status": "error", "updated_at": None,
                           "currencies": list(NBT_CODES.values()), "rates": {}}
    try:
        r = requests.get(NBT_API, timeout=15, headers={"User-Agent": USER_AGENT, "Accept": "application/json"})
        r.raise_for_status(); data = r.json(); out["status"] = "ok"
        out["updated_at"] = data.get("period_info", {}).get("date")
        for row in data.get("data", []):
            currency = NBT_CODES.get(str(row.get("Code", "")))
            if currency:
                out["rates"][currency] = {"rate": float(str(row.get("Rate", "0")).replace(",", ".")),
                                           "nominal": row.get("Nominal"), "date": row.get("Date")}
        missing = [c for c in NBT_CODES.values() if c not in out["rates"]]
        if missing: out["status"] = "partial"; out["missing"] = missing
...
def find_alif_quotes(value: Any, found: dict[str, dict[str, float | None]] | None = None):
    ...
    if isinstance(value, dict):
        code = next((str(value.get(k)).upper() for k in ("currency", "currencyCode", "code", "ccy", "symbol", "ticker")
                     if isinstance(value.get(k), str) and str(value.get(k)).upper() in CURRENCIES), None)
        if code:
            buy = sell = None
...
def alif_api() -> dict[str, Any]:
    ...
    try:
        ...
        r.raise_for_status(); data = r.json(); out["status"] = "ok"; out["rates"] = find_alif_quotes(data)
...
async def main() -> None:
    ...
    payload["reference_rates"] = {
        "collected_at": now_iso(), "currencies": list(NBT_CODES.values()), "nbt": nbt_rates(),
        "alif_api": alif_api(), "banks": await collect_bank_fx(),
        "note": "NBT is an official reference rate. USD/EUR bank quotes are monitoring data only until their rate class is verified.",
    }
    with open(RESULTS, "w", encoding="utf-8") as f: json.dump(payload, f, ensure_ascii=False, indent=2)
    print(json.dumps({"nbt_status": payload["reference_rates"]["nbt"]["status"],
                      "nbt_currencies_found": sorted(payload["reference_rates"]["nbt"]["rates"]),
                      "alif_api_status": payload["reference_rates"]["alif_api"]["status"],
                      "bank_sources": len(payload["reference_rates"]["banks"])}, ensure_ascii=False))
