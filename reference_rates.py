...
MAX_STALE_DAYS = 7
BANK_NAME_MAP = {
    "alif": "alif", "amonat": "amonatbank", "eskhata": "eskhata", "activ": "activbank", "humo": "humo",
    "international bank of tajikistan": "ibt", "ibt": "ibt", "oriyon": "oriyonbank", "spitamen": "spitamen",
    "vasl": "vasl", "dushanbe city": "dcity", "duşanbe city": "dcity", "d city": "dcity",
}
...
def parse_nbt_html(html: str) -> dict[str, Any]:
    ...
    try:
        ...
        for code, currency in NBT_CODES.items():
            ...
            if m:
                nominal = float(m.group(1)); rate = float(m.group(2).replace(",", "."))
...
def normalize_bank_name(name: str) -> str | None:
    ...
    for key, code in BANK_NAME_MAP.items():
        if re.sub(r"[^a-z0-9]+", " ", key.lower()).strip() in low:
            return code
    return None
...
def parse_nbt_commercial_html(html: str, currency: str) -> dict[str, Any]:
    from html import unescape
    out: dict[str, Any] = {"source": NBT_COMMERCIAL_HTML, "source_type": "official_nbt_commercial_cards", "status": "error", "currency": currency, "commercial_banks": {}}
    try:
        ...
        for row in rows:
            ...
            try:
                card_buy = float(clean[9].replace(",", ".")); card_sell = float(clean[10].replace(",", "."))
...
async def fetch_nbt_commercial(currency: str) -> dict[str, Any]:
    """Load NBT commercial-bank page and select USD/EUR in its currency control."""
    async with async_playwright() as p:
        ...
        try:
            ...
            if currency == "EUR":
                ...
                for i in range(count):
                    ...
                    for opt in options:
                        ...
                        value = (await opt.get_attribute("value") or "").upper()
            ...
            return parse_nbt_commercial_html(html, currency)
        finally:
            await browser.close()
...
def load_cached_nbt() -> dict[str, Any] | None:
    try:
        ...
        cached["status"] = "stale_fallback"; cached["source_type"] = "last_valid_official_nbt"; cached["stale"] = True
        cached["stale_age_days"] = round(age_days, 3); cached["current_scan_at"] = now_iso(); return cached
...
def merge_commercial_cache(current: dict[str, Any], cached: dict[str, Any] | None) -> tuple[dict[str, Any], bool]:
    ...
    merged = {k: dict(v) for k, v in cached_banks.items()}
    for bank_code, current_bank in current_banks.items():
        merged.setdefault(bank_code, {}).update(current_bank)
        merged[bank_code]["name"] = current_bank.get("name", merged[bank_code].get("name", bank_code))
    stale = []
    for bank_code in merged:
        for currency in ("USD", "EUR"):
            if not (merged[bank_code].get(currency) or {}).get("card_buy") and (cached_banks.get(bank_code, {}).get(currency) or {}).get("card_buy"):
                merged[bank_code][currency] = cached_banks[bank_code][currency]
                stale.append(f"{bank_code}:{currency}")
    current["commercial_banks"] = merged; current["commercial_banks_stale"] = sorted(set(stale))
    current["commercial_banks_source"] = "NBT current publication + last valid NBT publication per missing bank/currency"
    return current, bool(stale)


def nbt_rates() -> dict[str, Any]:
    errors: list[str] = []; fx: dict[str, Any] | None = None; fresh_fx = False
    try:
        r = requests.get(NBT_API, timeout=15, headers={"User-Agent": USER_AGENT, "Accept": "application/json"}); r.raise_for_status()
    ...
    for currency in ("USD", "EUR"):
        try:
            commercial_parts.append(asyncio.run(fetch_nbt_commercial(currency)))
        except Exception as exc:
            errors.append(f"Commercial {currency} {type(exc).__name__}: {exc}")
    current_commercial = {"commercial_banks": {}}
    for part in commercial_parts:
        for bank_code, data in part.get("commercial_banks", {}).items():
            current_commercial["commercial_banks"].setdefault(bank_code, {}).update(data)
    current_commercial, commercial_stale = merge_commercial_cache(current_commercial, cached)
    fx["commercial_banks"] = current_commercial["commercial_banks"]
    fx["commercial_banks_stale"] = current_commercial["commercial_banks_stale"]
    fx["commercial_banks_source"] = current_commercial["commercial_banks_source"]
    if fresh_fx:
        cache_nbt(fx)
    elif cached:
        # Preserve the cache timestamp; do not make stale data look fresh.
        pass
...
def alif_api() -> dict[str, Any]:
    ...
    try:
        r = requests.get(ALIF_API, timeout=15, headers={"User-Agent": USER_AGENT, "Accept": "application/json"}); r.raise_for_status(); data = r.json()
        out["status"] = "ok"; out["rates"] = find_alif_quotes(data)
...
async def main() -> None:
    ...
    payload["reference_rates"] = {"collected_at": now_iso(), "currencies": list(NBT_CODES.values()), "nbt": nbt_rates(), "alif_api": alif_api(), "bank_usd_eur_policy": "NBT commercial-bank Credit Cards Buy is the sole source for USD/EUR bank rates; last valid NBT publication is used for temporary gaps.", "note": "Separate bank USD/EUR scrapers are disabled. NBT official FX API/HTML and NBT commercial-bank table are the only USD/EUR sources."}
