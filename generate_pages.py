...
def main():
    # The collectors write their real output to site/results.json. Do not read
    # the obsolete data/normalized.json path: doing so makes the dashboard stale.
    data = load_json(SITE / "results.json", default={})
    calculated = load_json(SITE / "calculated_rates.json", default={})
    ...
    out = [
        ...
        "<style>body{font-family:system-ui,sans-serif;max-width:1200px;margin:32px auto;padding:0 16px;color:#222}table{border-collapse:collapse;width:100%;margin:12px 0 28px}th,td{padding:8px;border:1px solid #ddd;text-align:left;vertical-align:top}th{background:#f5f5f5}.muted{color:#666}.num{text-align:right;font-variant-numeric:tabular-nums}.warning{background:#fff7df}</style></head><body>",
    ...
    out.append("<p>USD/EUR bank values come only from the NBT commercial-bank Card Buy table. If a currency is absent from the current NBT table, it is not substituted and is not a publishable bank rate.</p>")
    ...
    out.append("<h2>RUB transfer calculations — coefficients + final result</h2>")
    out.append("<p>All coefficient calculations are shown here for audit/monitoring. Production tables receive only the final publishable rate.</p>")
    ...
    for row in [r for r in calculated.get("rates", []) if r.get("currency_code") == "RUB"]:
        status = str(row.get("status", ""))
        cls = "warning" if status in {"anomaly", "stale"} else ""
        out.append(f"<tr class='{cls}'><td>{html.escape(str(row.get('service_slug','')))}</td><td>{html.escape(str(row.get('bank_name') or row.get('bank_code','')))}</td><td>{html.escape(str(row.get('base_source_kind','')))}</td><td class='num'>{fmt_rate(row.get('base_rate'))}</td><td class='num'>{fmt_rate(row.get('coefficient'))}</td><td class='num'>{fmt_rate(row.get('raw_calculated_rate'))}</td><td class='num'><b>{fmt_rate(row.get('final_rate'))}</b></td><td>{html.escape(status)}</td></tr>")
    out.append("</table>")
    ...
    out.append("</table>")
    out.append("<p class='muted'>Alif API fallback is used only for RUB transfer-base calculation and never for USD/EUR bank publication.</p>")
